...
class ProblemSession:
    ...
    def import_problem_from_package(self, directory):
        problem_node = ElementTree.parse(os.path.join(directory, 'problem.xml'))
        if not problem_node:
            print('bad problem.xml file in ' + os.path.abspath(xmlfile.name))
            return
        if problem_node.find('tags'):
            print('tags:')
        ...
        def get_executable_options(node):
            ...
            options['checkExisting'] = 'true'
            options['sourceType'] = node.find('source').attrib['type']
            return options

        for solution_node in problem_node.find('assets').find('solutions').findall('solution'):
            ...
            try:
                ...
                print(e)
        for resource_node in problem_node.find('files').find('resources').findall('file'):
            ...
            try:
                ...
                print(e)
        for executable_node in problem_node.find('files').find('executables').findall('executable'):
            ...
            try:
                ...
                print(e)
        for checker_node in problem_node.find('assets').findall('checker'):
            ...
            try:
                ...
                print(e)
        for validator_node in problem_node.find('assets').find('validators').findall('validator'):
